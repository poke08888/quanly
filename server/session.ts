// Lightweight signed-cookie session for the BFF.
// Uses HMAC-SHA256 over APP_SECRET_KEY — no extra deps beyond 'cookie' (already installed).
...
export function setSession(res: Response, userId: number): void {
  res.cookie(COOKIE, sign(String(userId)), {
    httpOnly: true,
    secure: true,       // always HTTPS via Cloudflare edge
...
/** Express middleware: reject with 401 if no valid active session. */
export function requireAuth(req: Request, res: Response, next: NextFunction): void {
  const id = getSessionUserId(req)
  if (id !== null && getUser(id)?.active) { next(); return }
